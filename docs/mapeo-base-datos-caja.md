...
La columna `grupo` (`CJ`/`LA`/`RX`/`TM`/`EC`/`MM`, más `RM` sin ítems hoy) es la
categoría real del catálogo, catalogada en `GRUPO_NOMENCLATURA_ATENCION_MH`
(`CJ`=Caja/consultas y procedimientos, `LA`=Laboratorio, `RX`=Rayos X,
`TM`=Tomografías, `EC`=Ecografías, `MM`=Mamografía, `RM`=Resonancia Magnética). El
módulo nuevo usa esto para los chips de categoría del catálogo
(`App\Models\Caja\ItemCategory`).
...
### Anular un cobro (con aprobación)

La anulación **no es directa**: pasa por un flujo de aprobación que el sistema legado no
tenía (solo guardaba el resultado final, sin rastro de quién pidió ni quién autorizó).

1. El cajero abre el comprobante y elige "Solicitar anulación" → INSERT en
   `HSJ_Caja.void_requests` (base propia del aplicativo) con motivo de
   `Motivo_Anulacion_MH`, observación, y su usuario. **El comprobante sigue vigente.**
2. El **Jefe de Economía** o el **Cajero central** revisa la solicitud en
   `/caja/anulaciones`:
   - **Aprobar** → UPDATE `Cabecera_documento_MH` (`estado_doc='N'` +
     `cod_motiv_anu` del motivo elegido) dentro de una transacción, y la solicitud pasa
     a `approved` guardando quién aprobó, con qué rol y cuándo.
   - **Rechazar** → la solicitud pasa a `rejected` con observación obligatoria; el
     comprobante permanece vigente.

`void_requests` vive en `HSJ_Caja` (no en `SISGESH_BD`) porque es funcionalidad nueva;
sus referencias a documentos (SISGESH_BD) y usuarios (HSJ_Identity) son lógicas, sin FK,
por ser bases distintas.
