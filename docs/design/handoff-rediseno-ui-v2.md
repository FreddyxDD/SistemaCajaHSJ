...
## Overview
Rediseño visual del sistema de caja (Laravel + Livewire + Flux UI + Tailwind v4, repo `FreddyxDD/SistemaCajaHSJ`). Cubre: **landing pública**, login, panel, turno, nuevo cobro, comprobantes, anulaciones, cajeros, reportes, administración (usuarios + cajeros legado) y perfil, con modo claro/oscuro, logo institucional, footer y animaciones, con un color de marca elegido por el usuario.

## Cómo usar este paquete con Claude Code
Este ZIP es autosuficiente: entrégalo (o su contenido) a Claude Code en el repo `SistemaCajaHSJ` y pídele que implemente el rediseño descrito aquí sobre las vistas Blade reales — no que copie el HTML. El prototipo (`Sistema de Caja HSJ.dc.html`) y los logos (`assets/`) son la referencia visual exacta.
...
| --- | --- | --- |
| Landing pública | (nueva — página de entrada antes del login) | Nav con logo + "Iniciar sesión"/"Crear cuenta", hero centrado (badge, H1, copy, CTA) sobre orbes de fondo animados, 3 tarjetas de valor con tilt 3D, footer con crédito |
| Login | (nueva — no existía) | Tarjeta centrada 380px con logo, campo correo institucional + contraseña, botón primario ancho completo, enlace "Volver al inicio" |
...
## Assets
- `assets/logo.png` — logo institucional Hospital San José (variante para fondo claro).
- `assets/logo-dark.png` — variante para fondo oscuro.
- Usados en: nav de la landing, nav de la app, tarjeta de login. Se conmutan por CSS/estado según el tema activo (nunca filtros de color sobre el logo).
- Resto: iconos inline SVG (sol/luna, flecha, usuario, logout, turno, búsqueda, reportes) estilo Lucide/outline, ya en el prototipo.

## Footer (landing)
Texto exacto: `© 2026 Hospital San José de Chincha — Sistema de Gestión de Caja` (izquierda) y `Creado por Ing. FMC` (derecha), separado por un borde superior `--color-divider`, opacidad 0.6, 12px.

## Animaciones
- **Entrada del hero**: badge → título → copy → CTA en cascada, `hero-rise` (`translateY(22px)→0`, 0.6s, delays de 0.08s entre cada uno).
- **Logo**: `logo-spin-in` — `rotateY(90deg) scale(0.6)` → estado normal, 0.7s, con `perspective`.
- **Orbes de fondo**: `hero-float` flotando verticalmente, 7s loop, blur 40px, color del acento al 14–18%.
- **Tilt 3D en tarjetas de features**: `onMouseMove` calcula `rotateX/rotateY` según posición del cursor (±10°) + `translateY(-4px)`; se resetea en `onMouseLeave`. Requiere `perspective` en el contenedor grid y `transform-style: preserve-3d` en la tarjeta.
- **Hover genérico**: todas las `.card` interactivas usan `.lift` → `translateY(-4px)` + `box-shadow` mayor; botones tienen `:active { transform: scale(0.96) }`.
- **Barras del gráfico de 7 días**: `height` transiciona 0.8s al montar/cambiar datos.
- Ya existente: entrada general `win11-in` en tarjetas/diálogos/hijos de `<main>`.

## Files
- `Sistema de Caja HSJ.dc.html` — prototipo de referencia completo (todas las pantallas, tweak de color de marca).
- `assets/logo.png`, `assets/logo-dark.png` — logo institucional en ambas variantes.
